...
function findHighestConfidence(data) {
	...
	if (highestConfidence < 0.55) {
		...
		document.getElementById('title-human').innerHTML =
			'This image is generated by Human'
...
async function postToApi_url() {
	...
	const baseUrl = `https://results-api.optic.xyz/api/detector/reports/json?source=web&user_id=${visitorId}`
...
dropzone.addEventListener('drop', async function (event) {
	...
	if (fileSizeAllow == true) {
		if (
			...
			const baseUrl = `https://results-api.optic.xyz/api/detector/reports/raw?source=web&user_id=${visitorId}`
